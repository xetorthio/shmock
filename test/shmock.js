...
describe("shmock", function() {
  ...
  describe("Request", function() {
    ...
    it("Should record the URLs that were requestsed", function(done) {
      var handler = mock.get("*").persist().reply(200);

      test.get("/request1").expect(200, function() {
        handler.completedUrls().should.be.eql(['/request1']);
        test.get("/request2").expect(200).end(function(error, response) {
          handler.completedUrls().should.be.eql(['/request1', '/request2']);
          done();
        });
      });
    });
    ...
    it("Should be able to delay a reply for a specified amount of ms", function(done) {
      mock.get("/foo").persist().delay(30).reply(200);
